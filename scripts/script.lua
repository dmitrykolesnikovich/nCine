...
function ncine.on_init()
	...
	vp_texture0_ = nc.texture.init("Ping texture", nc.tex_format.RGB8, 1, resolution_int)
	vp_texture1_ = nc.texture.init("Pong texture", nc.tex_format.RGB8, 1, resolution_int)
...
void main()
{
	...
	vp_sprite0_ = nc.sprite.new(nil, vp_texture0_, resolution.x * 0.5, resolution.y * 0.5)
	vp_shaderstate0_ = nc.shaderstate.new(vp_sprite0_, blur_shader_)
	nc.shaderstate.set_uniform_float_value2(vp_shaderstate0_, "", "uResolution", resolution.x, resolution.y)

	vp_sprite1_ = nc.sprite.new(nil, vp_texture1_, resolution.x * 0.5, resolution.y * 0.5)
	vp_shaderstate1_ = nc.shaderstate.new(vp_sprite1_, blur_shader_)
	nc.shaderstate.set_uniform_float_value2(vp_shaderstate1_, "", "uResolution", resolution.x, resolution.y)

	nc.viewport.set_rootnode(scene_viewport_, rootnode)
	nc.viewport.set_rootnode(ping_viewport_, vp_sprite0_)
	nc.viewport.set_rootnode(pong_viewport_, vp_sprite1_)
...
function ncine.on_draw_viewport(viewport)
	if viewport == ping_viewport_ then
		nc.shaderstate.set_uniform_float_value2(vp_shaderstate0_, "", "uDirection", 1.0, 0.0)
		nc.shaderstate.set_uniform_float_value2(vp_shaderstate1_, "", "uDirection", 0.0, 1.0)
	end
end
...
function ncine.on_shutdown()
	...
	nc.shaderstate.delete(vp_shaderstate1_)
	nc.sprite.delete(vp_sprite1_)
	nc.shaderstate.delete(vp_shaderstate0_)
	nc.sprite.delete(vp_sprite0_)
...
function setup_viewport()
	...
	if with_viewport_ then
		...
		nc.viewport.set_rootnode(nc.application.get_screen_viewport(), vp_sprite1_)
	else
